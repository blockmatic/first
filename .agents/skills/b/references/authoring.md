...
One outcome per slash playbook. Keep discovery descriptions to capability and invocation; detailed steps belong in the body. The frontmatter name equals its folder, starts with `b-`, and retains `disable-model-invocation: true`.
...
Small existing specialty playbooks may use Purpose, Steps, and Completion. Add detailed checklists when improving their behavior, not merely to fill headings. Supporting files belong inside `b/references/` so installing `b` carries them. Other catalogs are optional context, never install-time dependencies.
