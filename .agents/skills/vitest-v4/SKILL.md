...
- Does not cover app Playwright E2E — see [playwright-v1](../playwright-v1/SKILL.md) for ad-hoc browser automation; app E2E conventions live in consuming-repo rules
- HTTP app tests with `inject()` and database isolation (PGLite, `*.spec.ts` vs `*.test.ts`) are consuming-repo concerns — follow the project's Fastify testing rule, not this skill alone
